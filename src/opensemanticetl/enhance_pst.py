...
class enhance_pst(object):
	...
	def process (self, parameters={}, data={} ):
		...
		if mimetype.lower().startswith('application/vnd.ms-outlook-pst') or filename.lower().endswith('.pst'):
			if verbose:
				print ("Mimetype ({}) or file ending seems Outlook PST file, starting extraction of emails".format(mimetype) )

			self.pst2email(pstfilename=filename, parameters=parameters, verbose=verbose)
	...
	def pst2email(self, pstfilename, parameters={}, verbose=False):
		...
		temp_dirname = system_temp_dirname + os.path.sep + "opensemanticetl_enhancer_pst_" + os.getpid() + "_" + h.hexdigest()
