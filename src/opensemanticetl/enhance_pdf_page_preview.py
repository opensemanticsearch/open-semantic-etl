...
class enhance_pdf_page_preview(object):
	...
	def segment_pdf_to_pages ( self, parameters={}, data={} ):
		...
		if verbose:
			print ("Generating single page PDF for previews from {} for {} to {}".format(filename, docid, thumbnail_dir + thumbnail_subdir))


		# call pdftk burst
		try:
			result = subprocess.check_call(['pdftk', filename, 'burst', 'output', thumbnail_dir + thumbnail_subdir + '/%d.pdf'])
			data['etl_thumbnails_s'] = thumbnail_subdir
		except BaseException as e:
			sys.stderr.write( "Exception while genarating single page PDFs by pdftk burst\n")
	...
	def process (self, parameters={}, data={} ):
		...
		if "application/pdf" in mimetype.lower() or filename.lower().endswith('.pdf'):
			if verbose:
				print ( 'Mimetype or filename suffix is PDF ({}), extracting single pages for segmentation)'.format(mimetype) )
