...
def pdfimages2text(filename, lang='eng', verbose=False, pdf_ocr=True, pdf_ocr_descew=False):
    ...
    result = subprocess.call(
        ['pdfimages', '-p', '-j', filename, ocr_temp_dirname + os.path.sep + 'image'])
    ...
    if result == 0:
        ...
        for image in images:
            ...
            if pdf_ocr:
                ...
                try:
                    ...
                    sys.stderr.write(
                        "Exception while OCR of PDF: {} - maybe corrupt image: {} - exception: {}".format(filename, imagefilename, e))
            ...
            if pdf_ocr_descew:
                ...
                try:
                    ...
                    sys.stderr.write(
                        "Exception while optimized ocr pdf: {} - maybe corrupt image: {} - exception: {}".format(filename, imagefilename, e))
        ...
        sys.stderr.write(
            "Error: Extracting images from PDF failed for {} {}".format(filename, result))
...
def enrich_pdf(parameters={}, data={}):
    ...
    try:
        ocr_txt, ocr_optimized_txt = pdfimages2text(
            filename=filename, lang=lang, verbose=verbose, pdf_ocr=True, pdf_ocr_descew=pdf_ocr_descew)
...
class enhance_pdf_ocr(object):
    ...
    def process(self, parameters={}, data={}):
        ...
        if "application/pdf" in mimetype.lower() or filename.lower().endswith('.pdf'):
            if verbose:
                print('Mimetype is PDF ({}) or file ending is PDF, starting OCR of embedded images'.format(
                    mimetype))
