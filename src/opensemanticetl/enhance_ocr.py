...
def image2text(filename, lang='eng', verbose=False, cache=None):
    ...
    #
    if cache:
        # calc filename from cache dir and file content not filename,
        # which will be different temp filename in future
        md5hash = hashlib.md5(
            open(filename, 'rb').read().encode('utf-8')).hexdigest()
        ...
        ocr_temp_filename = tempfile.gettempdir() + os.path.sep + \
    ...
    if cache and os.path.isfile(ocr_temp_filename + '.txt'):
        ...
        if verbose:
            print('Found in OCR cache, so reading OCR results for {} from {}'
                  .format(filename, ocr_temp_filename))
    ...
    # not in cache, so call Tesseract OCR
        ...
        result = subprocess.call(['tesseract', '-l', lang, '--user-words',
                                  '/etc/opensemanticsearch/ocr/dictionary.txt',
                                  filename, ocr_temp_filename])

        if result != 0:
            sys.stderr.write("Error: OCR failed for {}\n".format(filename))
