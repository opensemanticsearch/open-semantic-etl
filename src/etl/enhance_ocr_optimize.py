...
def optimized_image2text(filename, lang='eng', verbose=False):
	...
	ocr_temp_dirname = tempfile.mkdtemp(prefix="opensemanticetl_ocr_optimized_")
	...
	if result == 0:
		...
		for image in images:
			...
			try:
				...
				sys.stderr.write( "Exception while ocr optimized image of: {} - Maybe optimized image {} corrupt? Exception: {}\n" .format(filename, imagefilename, e.message) )



	else:
		sys.stderr.write ( "Error: Optimizing images for ocr failed for {} with return code {}".format(filename, result) )
...
class enhance_ocr_optimize(object):
	...
	query = "content_type: image\/* AND NOT enhance_ocr_optimize_b:true"
	...
	def process (self, parameters={}, data={} ):
		...
		if "image" in mimetype.lower():
			if verbose:
				print ("Mimetype seems image ({}), starting optimized OCR".format(mimetype) )
		
			ocr_txt = optimized_image2text(filename, lang, verbose)
			
			if ocr_txt:
				data['ocr_optimized_t'] = ocr_txt
	
			# mark the document to indicate, that it was analyzed by this plugin
			data['enhance_ocr_optimize_b'] = "true"
