...
def getmeta_modified(metaservers, docid, verbose=False):
	...
	try:
		...
		sys.stderr.write( "Exception while getting metadata modification time: {}\n".format(e.message) )
	...
	if verbose:
		...
		for subj, pred, obj in g:
				...
				try:					
					...
					sys.stderr.write( "Exception while printing triple: {}\n".format(e.message) )
		...
		try:
			...
			for subj, pred, obj in g.triples((None, facetRef, None)):
				try:
					...
					if verbose:
						print ( "Adding Solr facet {} with the object {}".format( solr_facet, obj ) )
					...
					sys.stderr.write( "Exception while checking predicate {}{}\n".format(pred, e.message) )

		except BaseException as e:
			sys.stderr.write( "Exception while checking a part of metadata graph: {}\n".format( e.message ) )


		
	# if semantic mediawiki modification date field, take this as date
	moddateRef = URIRef("http://semantic-mediawiki.org/swivt/1.0#wikiPageModificationDate")
	...
	def process (self, parameters={}, data={} ):
		...
		if isinstance(metaserver, basestring):
			...
			for server in metaserver:
				...
				data = getmeta_rdf_from_server(metaserver=server, data=data, property2facet=property2facet, docid=docid, verbose=verbose)
