	...
	def process (self, parameters={}, data={} ):
		...
		if xmpfilename:
			...
			try:
				...
				# get location
				try:
					location = root.findtext(".//{http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/}Location")
					...
					sys.stderr.write( "Exception while parsing location from xmp {} {}".format(xmpfilename, e.message) )
				...
				try:
					for tag in root.findall(".//{http://purl.org/dc/elements/1.1/}subject/{http://www.w3.org/1999/02/22-rdf-syntax-ns#}Bag/{http://www.w3.org/1999/02/22-rdf-syntax-ns#}li"):
						try:
							...
							sys.stderr.write( "Exception while parsing a tag from xmp {} {}".format(xmpfilename, e.message) )
				except BaseException as e:
					sys.stderr.write( "Exception while parsing tags from xmp {} {}".format(xmpfilename, e.message) )
	
	
			except BaseException as e:
				sys.stderr.write( "Exception while parsing xmp {} {}".format(xmpfilename, e.message) )
